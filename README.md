...
- **Inject (which skill to use):** `/oc-plan` can tag each sub-task with the relevant skill(s); `/oc-exec` passes them into the OpenCode prompt (`Available skills: ...`) so the executor knows to invoke them. Claude picks **instruction-only** skills per task (e.g. `be-logging-convention`, `be-databases`) and avoids skills that need Claude-Code-only tools (`AskUserQuestion`, Task subagents, the Skill tool, `/opencode-plugin-cc:*` commands) - those can't run in OpenCode's headless mode.
